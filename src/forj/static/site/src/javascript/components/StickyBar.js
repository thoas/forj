class StickyBar {
  ...
  init() {
    ...
    this.target.parentNode.insertBefore(
      this.shadow_bar_before,
      this.target.previousSibling
    );
  }
  ...
  check_height() {
    if (this.stick_to === "top") {
      ...
      if (
        bound_after < this.height_target &&
        this.stick_ready &&
        !this.unstick_ready
      ) {
        ...
        bound_before > -this.height_target &&
        !this.stick_ready &&
        this.unstick_ready
      ) {
        this.stick_ready = true;
      ...
      if (
        bound_after < window.innerHeight &&
        this.stick_ready &&
        !this.unstick_ready
      ) {
        this.stick_ready = false;
