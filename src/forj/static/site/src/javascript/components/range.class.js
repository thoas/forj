...
class Range {
  ...
  update_bancs(){
      ...
      if (window.GL.bancs.length === 2) { add_banc.classList.add('disabled') }
      if (window.GL.bancs.length === 0) { remove_banc.classList.add('disabled') }
  }
  ...
  init_bancs(){
      ...
      add_banc.addEventListener('click', ()=> {
          ...
          if (window.GL.bancs.length < 2) {
              ...
              if (window.GL.bancs.length === 2) { add_banc.classList.add('disabled') }
              else { add_banc.classList.remove('disabled'); remove_banc.classList.remove('disabled')  }
          }
      })
  ...
  update_labels(){
      ...
      if (window.STORAGE.TABLE.outside) { outside.innerHTML = "<br>Traitement extérieur" }
      else { outside.textContent = "" }

      let bancs = document.querySelector('section.infos .bancs')
      if (window.GL.bancs.length > 0) { bancs.textContent = "(" + window.GL.bancs.length + ")" }
      else { bancs.textContent = "(0)" }

      let outside_popin = document.querySelector('.basket .outside')
      if (window.STORAGE.TABLE.outside) { outside_popin.innerHTML = "<br>Traitement extérieur" }
      else { outside_popin.textContent = "" }

      let bancs_popin = document.querySelector('.basket .bancs')
      if (window.GL.bancs.length > 0) { bancs_popin.textContent = "(" + window.GL.bancs.length + ")" }
      else { bancs_popin.textContent = "(0)" }
