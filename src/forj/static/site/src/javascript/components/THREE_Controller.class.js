...
class THREE_Controller {
  ...
  init_loader() {
    ...
    this.assets.hdr = new THREE.TextureLoader(this.manager).load(
      staticUrl + "site/build/assets/images/hdr.png"
    );
    this.assets.shadow = new THREE.TextureLoader(this.manager).load(
      staticUrl + "site/build/assets/images/shadow.png"
    );
  ...
  init_event() {
    ...
    this.container.addEventListener("mousemove", function(event) {
      ...
      if (that.drag) {
        ...
        that.drag_rotation.y = Math.min(
          Math.max(that.drag_rotation.y, Math.PI / 20),
          Math.PI / 4
        );
      }
  ...
  update() {
    ...
    this.rotation_direction.subVectors(
      this.drag_rotation,
      this.camera_rotation
    );
    this.rotation_direction.multiplyScalar(0.04);
    this.camera_rotation.addVectors(
      this.camera_rotation,
